...
::: {.callout-important}
It might sound counter intuitive, but most real-world data IS NOT stored in a tidy format!
...
### OK, you are ready to start a project and loaded some data into R, but they are untidy! What should you do to tidy them up?
...
::: {.callout-note}
The tidyr package includes many other functions, but we focus on the most important. Each behaves a bit differently, so be sure to check the documentation for details on their arguments.
:::
...
::: {.fragment}
- *"Each variable must have its own column"*: the current values of `type` are actually variables names 
...
<!--
Each observation must have its own row: each country-year observation is split across two rows because of type.
--> 
...
## Use Pipes to Chain tidyr functions
