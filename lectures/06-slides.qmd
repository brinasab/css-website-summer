...
2. **Graph:** To answer this, we need to show the relationship between *two numeric variables* and compare it across *one categorical variable*
...
### dplyr (pronounced “d-ply-r” letter by letter or “dee-plyer”):

- Designed to **clean, organize, and summarize data**

- Makes it easier to prepare data for **better graphs**

- Includes several functions or **verbs**: we learn the most important this week
...
Now we will use two questions to practice choosing the right `dplyr` functions and translating plain-language data questions into code.
...
1. Identify the data and variables (columns) we need → `penguins`, `species`, `body_mass_g` 
...
```
x <- 5                  # use <- to assign 5 to an object
...
```
x == y    # is x equal to y? (TRUE or FALSE)
...
We use OR to keep rows where a variable is equal to **one value OR another value**. In R, we can write this in two equivalent ways:

<br>
Option 1: use `|` to spell out each condition
...
Option 2: use `%in%` to check whether a value is in a list of allowed values
...
Share your code here: <https://codeshare.io/5zlNLE> and answer the poll everywhere question from the chat once you are done
...
- often used with categorical variables
:::
:::
...
## Pipes are the best but there are multiple ways to write R code
...
⚠️ **Drawback: You must save each intermediate object.** This can clutter your environment, increase R memory usage, and make your code more prone to typos. Shorter names for each step might reduce typos but sacrifice readability.
...
**⚠️ Drawback: It overwrites the original dataset.** If something goes wrong, you need to re-run everything.
