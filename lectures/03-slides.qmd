...
Paste this code into your R Console and run it to see the `mpg` dataset (use most recent R version):
...
- The code `library(ggplot2)` loads the `ggplot2` package in our current R session: 
 - A package is a collection of code, data, and documentation
 - What's the difference between loading and installing a package?
- Check your R version on Workbench: use the most recent for data viz!   
...
- What's on the x-axis and y -axis? What does each dot in this scatterplot represent?
- You want a car with high fuel efficiency on the highway and a large engine size. Can you find one?
...
Run the code, and answer these questions:

- What do you see? How is this new plot different from the previous one? 
...
- Check the [documentation for `geom_point()`](https://ggplot2.tidyverse.org/reference/geom_point.html) to learn about its aesthetics
...
We’ve been creating our first plots intuitively. Let’s formally key concepts:

- `ggplot2` is the name of the main R package for **data visualization** <https://ggplot2.tidyverse.org/>
- It’s part of the `tidyverse`, a collection of packages for data analysis: <https://www.tidyverse.org/>
- Created by **Hadley Wickham**, who also wrote our course textbook
- Built using the theory called the **Grammar of Graphics** a system for creating layered plots (see [this paper](https://www.tandfonline.com/doi/pdf/10.1198/jcgs.2009.07098) for an intro to it)
...
You can also add more components, like:
...
Now, we fill in this code template using the `mpg` data and plot it:

:::{.columns}
::: {.column width="50%"}

```r
# code template (previosu slide)
...
stat = "identity" mean use the y-values exactly as they are in my data. Do not calculate them for me.
This often comes up with bar plots with geom_bar

By default, geom_bar() counts rows:
...
In the next slides, we’ll take a closer look at a few *selected aspects* of these components. For more details, see today’s readings.

::: callout-important
💡  The following slides focus on the most common aspects you’ll encounter as you get started with ggplot2. Start from these. You’ll pick up the rest as you experiment and build more plots.
:::
...
**Geom:** the type of plot you make, which depends on your variables type. Most common (memorize them!):
...
**Stat:** transforms your data for plotting and depends on which geometric object you are using (e.g., the type of graph). 
