...
::: {.callout-tip}
The **Tidyverse** is often the most intuitive place to start when learning R and you’ll keep using it long after this course, but **base R** helps us understand how R works underneath.
:::
...
Today we begin a **new module on [Base R](https://brinasab.github.io/css-website/lectures/)** What you will learn in these lectures:
...
# bring in second df
# all = FALSE keeps only states that appear in both data frames
tw_full <- merge(tw, state_lookup, by = "state", all = FALSE)

# compute the total sum of likes + retweets
sum_inter <- sum(tw_full$tot_likes_retweets)

# compute the sum of likes + retweets by each vote category
