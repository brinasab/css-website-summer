...
<!--
UPDATE FOR FALL 2026 ON FACTORS:
ADD ONE EXAMPLE WHEN THEN NEED TO PASS THE LABELS AND LEVELS NOT BY HAND USING DISTINCT() OR UNIQUE()
SEE THIS AS EXAMPLES 
https://github.com/css-summer26/hw03-carolinaporto/blob/main/scotus.md
https://github.com/css-summer26/hw03-sara-jayakumar/tree/main
-->
...
- **Explicit missing data**: the missing value is recorded in the data, typically as `NA` (Not Available) 
- **Implicit missing data**: the missing value was never recorded in the data

We focus on explicit missing data. For implicit missing data, see [R for Data Science Ch. 18](https://r4ds.hadley.nz/missing-values)
...
Any operation involving a missing value will return a missing value (see [Ch. 12.2.2 Missing values](https://r4ds.hadley.nz/logicals#sec-na-comparison)):
...
**Use `is.na()` to find missing values.** It returns `TRUE` when a value is missing, and `FALSE` when is present.
...
To pull out the rows where a value is missing, **use `is.na()` inside `filter()`** to keep rows where a value is missing or is not missing, as follows:
...
:::{.callout-note}
You cannot use `== NA` to check for missing values: `NA` means “unknown,” so R cannot evaluate it. Use `is.na()` instead.
:::
...
:::{.callout-warning}
Be careful, `drop_na()` removes rows from the data you are working with.
It removes the entire row, not just the missing cell. This can accidentally remove useful data from other variables in the same row. Before using it, check the variable (see previous slides).
:::
...
- First, **rename:** use the `penguins_raw` dataset and rename `Flipper Length (mm)` to `flipper_length_mm`. Save the result as a new dataframe (call it `penguins_clean` or `p`)

- Then, use the new dataframe for the steps below (copy/adapt code from these slides):  
  - **Detect missing values:** use `is.na()` and `sum()` to count how many are missing in flipper length
  - **Exclude from calculations:** use `na.rm = TRUE` inside `mean()` to calculate the average flipper length
...
Beloa are the main functions in R to replace or fill missing values:
...
## Task: Plot the total number of schools in the U.S. by school type
