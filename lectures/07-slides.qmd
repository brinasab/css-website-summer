...
| **Columns** | `rename()` renames columns; `relocate()` reorders columns | `mutate()` adds or changes column values — as central as `filter()`, `group_by()`, `summarize()`; `across()` applies one operation to many columns at once |
| **Groups** | `group_by()` defines groups; `summarize()` summarizes each group | `count()` — shortcut for `group_by()` + `summarize(n())` |
