...
Can our code skip some lines and run others depending on a condition? 

Can it repeat itself when needed? 
...
*Conditional statements and loops are control structures. Today we focus on conditional statements. Next lecture we focus on loops*

<!-- 
They let our code responds dynamically to different situations
...
Conditional statements translate the **“if this, then that” logic into code**: R checks whether a condition is true or false and acts accordingly,

We learn the following:

> 1. **Single test with `if... else`** *If this is true → do this. Otherwise → do that*
>
> 2. **Multiple tests with `if... else if... else`**  *If A → do this. Else if B → do that. Else → fallback*
>
> 3. **Nested tests with one `if` inside another** *First check one condition, then check another condition inside it*
>
> 4. **Vectorized tests with `ifelse()`** *Apply “if this, then that” logic to each value in a vector* 

<!--
The last one is not technically a conditional statement like the previous three, but good for vectorized logic
...
2. Open the file called `if-else-warm-up.Rmd`

3. Before running each code, try to predict what it will output
...
But omitting them is considered **bad style**: it hurts readability and makes errors more likely, especially for more complex statements.
...
Note conditional statements evaluate only one single element at a time. In this example, the mean of a vector.
...
## 2. Multiple tests: if / else if / else

Use `if... else if... else` when your code tests **multiple conditions** and takes different actions depending on which is `TRUE`. To do so, add one or more **`else if`** statement(s) between the initial `if` and the final `else`
...
- Order your conditions from most specific to most general, ending with a default case `else`
:::


---

### 💻 Practice 1: What happens if the conditions are in the wrong order? 

Copy the code below in your console. Move the line `temperature >= 10` before the line `temperature >= 20`. What do you notice?
...
Once done, [share your code here](https://codeshare.io/G8nxLD){target="_blank"}. 
...
### 💻 Practice 2: Add a new condition

Copy the code below into your console. Add one more `else if` statement that checks whether the temperature is at least 0 but less than 10. If that condition is true, set `weather <- "Cold"`. Leave the input temperature set as 4.
...
1. Think about where the new condition belongs in the `if / else if / else` flow

2. Then ask yourself: how do I write “at least 0 and less than 10” in R? Should you use `&` or `|`?
...
### 💻 Practice 3: When should we use and versus or?

Run the two examples below. Compare the outputs and try to explain why they are different.
...
<!--
With AND: BOTH conditions must be TRUE (here, >= 0 and < 10) at the same time for the code to be evaluated as TRUE
Here temperature must be greater than or equal to 0, and temperature must be less than 10. That’s what defines our “Cold” range (0–9°C).

If you write OR only ONE CONDITION must be TRUE for the code to run. 
So this condition is TOO BROAD would always be true, because every number is either ≥ 0 or < 10.
...
## 3. Nested tests: one if statement inside another
...
What output do you expect if `x <- 0`? 
...
Our final case for today is `ifelse()` which *looks like a conditional statement, but it's actually a function* that applies a condition to **each element of a vector**. 
...
Another example, but this time with a vector as input. What is the output of this code?
...
Because it works element-wise `ifelse()` it is useful for creating or changing values in a data frame column.
...
Nnote in the base R code, u can also use double square bracket to access column Scores by position vs name 
...
# stick with [[]] for vector operations, safer always returns a vector


# Tidyverse subsetting: select() returns a tibble
qualify %>%
  select(Scores)

# Tidyverse subsetting: pull() returns a vector
qualify %>%
  pull(Scores)

**Tidyverse takeaway:**  
Use `select()` when you want to keep a column as a data frame/tibble.  
Use `pull()` when you want to extract a column as a vector.
Since `ifelse()` works element by element, it needs a vector of TRUE/FALSE values.


This is similar to the base R difference between `[ ]` and `[[ ]]`:
- `[ ]` keeps the result as a data frame/list-like object
- `[[ ]]` extracts the actual vector
- In tidyverse, `select()` keeps a tibble, while `pull()` extracts a vector
...
In the previous example, we only had two possible categories: "Admitted" and "Rejected." What happens to our code if we add a third category: "Waitlisted"? 

We could write a **nested `ifelse()`** like this:
...
Nested `ifelse()` statements do the job, but they make the code harder to read. 
...
## 4. Using ifelse() to recode variables in a dataframe

You might run into code that uses `ifelse()` to recode variables in a dataframe. 

For example, the variable `decisionDirection` takes four values: *1 = Conservative; 2 = Liberal; 3 = Other; NA*.  The code below recodes it to three: *0 = Conservative; 1 = Liberal; NA for both NA and Other*.  

```r
data |>
  mutate(decision_direction = ifelse(
...
::: {.callout-tip title="Tip"}
Prefer `case_when()` over `ifelse()` for recoding variables: it is easier to read and scale. 
:::
...
![](15-ifelse-recoding.png)

---

## 4. Vectorized tests with ifelse() 

**Note there is also a tidyverse version of the base R `ifelse()` function, called `if_else()` <https://dplyr.tidyverse.org/reference/if_else.html>**

The two are similar, hence I introduced only the base R version here.

In similar past cases (e.g., `read_csv()` vs. `read.csv()`), I suggested defaulting to the tidyverse, and that guidance still generally holds. The tidyverse version `if_else()` does a slightly better job warning us against missteps, but `ifelse()` is widely used.

---

## 💻 Practice

Download additional practice exercises from the course website.
