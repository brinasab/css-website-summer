...
Start from what we know... Make a **scatter plot with two numeric variables**

Copy and paste this code in R:
...
We generated these plots with ggplot2 but today we introduce a second key package from the tidyverse: **[dplyr](https://dplyr.tidyverse.org/) for data manipulation.**

- Designed to **clean, organize, and summarize data**

- Makes it easier to prepare data for **better graphs**

- Includes several intuitive functions for common data tasks. We learn the most important ones this week.
...
## Key dplyr functions we learn today


| Task | Function |
|---|---|
| Keep or reorder columns | `select()` |
| Keep rows based on conditions | `filter()` |
| Calculate summaries by sub-groups | `group_by()` |
| Calculate summaries, such as averages | `summarize()` |


::: {.callout-tip}
## Programming as problem solving

We will work through two questions together that demonstrates what I mean by "programming as problem solving" and require us to use these dplyr functions!
:::
...
::: {.callout-note title="From Question to Code: Practice translating plain-language data questions into dplyr code."}
...
| `y >= x` | is `y` greater than or equal to `x`? |
...
The package `dplyr` has many functions. These functions:
   - Act *like verbs* in a sentence: they express *what to do* with the data
...
Say some more on select, filter, summarize, then transition (next slides) to group_by
